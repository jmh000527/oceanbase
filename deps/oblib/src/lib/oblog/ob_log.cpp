    ...
    for (i = 0; OB_SUCC(ret) && i < config.n_way_; i++) {
      if (OB_UNLIKELY(NULL != config.prealloc_items_[i])) {
        ...
        if (OB_ISNULL(config.prealloc_items_[i])) {
          ...
          LOG_STDERR("allocate prealloc_plog_items failed, ret=%d, total_size=%ld, log_size=%d",
...
int ObLogger::alloc_log_item(const int32_t level, const int64_t size, ObPLogItem *&log_item)
{
  ...
  if (!stop_append_log_) {
    void *vlog_item = NULL;
    bool is_large = false;
    int64_t slot = INT64_MAX;

    if ((size - LOG_ITEM_SIZE) > MAX_LOG_SIZE) {
    } else {
      is_large = (size - LOG_ITEM_SIZE) > BASE_LOG_SIZE ? true : false;
      slot = is_large ? (common::get_itid() % LARGE_LOG_POOL_N_WAY) :
