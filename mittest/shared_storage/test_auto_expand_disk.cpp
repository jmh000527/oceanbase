...
TEST_F(TestAutoExpandDisk, test_auto_expand_disk_task)
{
  ...
  ASSERT_EQ(datafile_size-total_disk_size, OB_SERVER_DISK_SPACE_MGR.get_free_disk_size());

  // 1.alloc disk_size reached 90% of reserved_size
  int64_t disk_size = tenant_disk_space_mgr->get_private_macro_reserved_size() * ObTenantDiskSpaceManager::DEFAULT_TENANT_DISK_SIZE_EXPAND_THRESHOLD_PCT + 1;
  ...
  ASSERT_TRUE(tenant_disk_space_mgr->need_expand_disk_size());
