...
static const char *TEST_FILE_NAME = "test_tx_recover3";
static const char *BORN_CASE_NAME = "ObTxBeforeRestartTest3";
static const char *RESTART_CASE_NAME = "ObTxAfterRestartTest3";
...
class ObTxBeforeRestartTest3 : public ObSimpleClusterTestBase
{
public:
  ObTxBeforeRestartTest3() : ObSimpleClusterTestBase(TEST_FILE_NAME) {}
...
TEST_F(ObTxBeforeRestartTest3, recover_tx_ctx_before_memtable)
{
  ObSqlString sql;
  int64_t affected_rows = 0;
  ...
  WRITE_SQL_FMT_BY_CONN(connection, "insert into test_tx_recover values(2);");
  ...
  // ============================== freeze tx ctx memtable ==============================
  minor_freeze_tx_ctx_memtable(ls);

  ASSERT_EQ(tx_ctx_memtable->get_rec_scn(), share::SCN::max_scn());

  // ============================== freeze data memtable ==============================
  minor_freeze_data_memtable(ls);

  ASSERT_EQ(tx_ctx_memtable->get_rec_scn(), share::SCN::max_scn());

  // ============================== commit the txn ==============================
  WRITE_SQL_BY_CONN(connection, "insert into test_tx_recover values(4);");
  ...
  // ============================== freeze tx ctx memtable ==============================
  // minor_freeze_tx_ctx_memtable(ls);
  // ASSERT_EQ(tx_ctx_memtable->get_rec_log_ts(), INT64_MAX);
}

class ObTxAfterRestartTest3 : public ObSimpleClusterTestBase
{
public:
  ObTxAfterRestartTest3() : ObSimpleClusterTestBase(TEST_FILE_NAME) {}
};

TEST_F(ObTxAfterRestartTest3, recover_tx_ctx_before_memtable_recover)
{
  ObSqlString sql;
  int64_t affected_rows = 0;
  ...
  ASSERT_EQ(OB_SUCCESS, sql_proxy.acquire(connection));
  ASSERT_NE(nullptr, connection);
  WRITE_SQL_FMT_BY_CONN(connection, "insert into test_tx_recover values(5);");
