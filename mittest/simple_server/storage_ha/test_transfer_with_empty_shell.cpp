...
public:
  ...
  int wait_transfer_task(
      ...
      ObTransferTask &task);
  int wait_transfer_out_deleted_tablet_become_empty_shell(
...
int TestTransferHandler::wait_transfer_out_deleted_tablet_become_empty_shell(
    const ObTransferTask &task)
{
  int ret = OB_SUCCESS;
...
TEST_F(TestTransferHandler, test_transfer_1001_to_1002)
{
  ...
  sql.reset();
  ASSERT_EQ(OB_SUCCESS, sql.assign_fmt("set ob_global_debug_sync = 'BEFORE_TABLET_GC wait_for signal execute 10000'"));
  ASSERT_EQ(OB_SUCCESS, inner_sql_proxy.write(OB_SYS_TENANT_ID, sql.ptr(), affected_rows));
