...
static const char *TEST_FILE_NAME = "test_transfer_doing_stage_restart_with_mds_flush";
...
TEST_F(TestTransferHandler, test_transfer_1001_to_1_with_transfer_start_with_mds_flush)
{
  int ret = OB_SUCCESS;
  ...
  sql.reset();
  ASSERT_EQ(OB_SUCCESS, sql.assign_fmt("set ob_global_debug_sync = 'BEFORE_CHECKPOINT_TASK wait_for signal execute 10000'"));
  ASSERT_EQ(OB_SUCCESS, inner_sql_proxy.write(OB_SYS_TENANT_ID, sql.ptr(), affected_rows));
  usleep(100000); // wait for debug_sync_timeout to take effect

  sql.reset();
  ASSERT_EQ(OB_SUCCESS, sql.assign_fmt("set ob_global_debug_sync = 'SWITCH_LEADER_BEFORE_TRANSFER_DOING_START_TRANS wait_for signal1 execute 10000'"));
  ASSERT_EQ(OB_SUCCESS, inner_sql_proxy.write(OB_SYS_TENANT_ID, sql.ptr(), affected_rows));
  usleep(100000); // wait for debug_sync_timeout to take effect
  ...
  ASSERT_EQ(OB_SUCCESS, wait_start_transfer_tablet_mds_flush(g_tenant_id, task));
}

TEST_F(TestTransferRestart, observer_restart_when_transfer_start_without_flush)
{
