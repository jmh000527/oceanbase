...
int ObTableTransCtrl::start_trans(ObTableTransParam &trans_param)
{
  int ret = OB_SUCCESS;
  NG_TRACE(T_start_trans_begin);

  if (trans_param.is_readonly_) {
    if (OB_FAIL(init_read_trans(trans_param))) {
      LOG_WARN("fail to init read trans", K(ret), K(trans_param));
    }
  } else if ((!trans_param.is_readonly_) && (ObTableConsistencyLevel::EVENTUAL == trans_param.consistency_level_)) {
...
int ObTableTransCtrl::end_trans(ObTableTransParam &trans_param)
{
  int ret = OB_SUCCESS;
  NG_TRACE(T_end_trans_begin);

  if (trans_param.is_readonly_) {
    release_read_trans(trans_param.trans_desc_);
  } else {
