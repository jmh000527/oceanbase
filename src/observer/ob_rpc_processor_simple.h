...
OB_DEFINE_PROCESSOR_S(Srv, OB_DETECT_CLIENT_SESSION_ALIVE, ObRpcDetectClientSessionAliveP);
...
OB_DEFINE_PROCESSOR_S(Srv, OB_CLONE_KEY, ObCloneKeyP);
OB_DEFINE_PROCESSOR_S(Srv, OB_TRIM_KEY_LIST, ObTrimKeyListP);
#endif
...
OB_DEFINE_PROCESSOR_S(Srv, OB_SERVER_SYNC_BASELINE, ObServerSyncBaselineP);
#endif
...
OB_DEFINE_PROCESSOR_S(Srv, OB_FETCH_TABLET_PHYSICAL_ROW_CNT, ObRpcFetchTabletPhysicalRowCntP);
OB_DEFINE_PROCESSOR_S(Srv, OB_CHECK_AND_CANCEL_DELETE_LOB_META_ROW_DAG, ObRpcCheckandCancelDeleteLobMetaRowDagP);
OB_DEFINE_PROCESSOR_S(Srv, OB_HA_REBUILD_TABLET, ObRebuildTabletP);
OB_DEFINE_PROCESSOR_S(Srv, OB_CHECK_SYS_TABLE_SCHEMA, ObCheckSysTableSchemaP);
