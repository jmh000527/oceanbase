  ...
  ObMySQLTransaction &trans)
{
  ...
  if (OB_ISNULL(ddl_service_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < sensitive_rules.count(); i++) {
      ...
      // for (int64_t j = 0; OB_SUCC(ret) && j < hidden_table_schema.get_column_count(); ++j) {
      //   const ObColumnSchemaV2 *hidden_col_schema = hidden_table_schema.get_column_schema_by_idx(j);
      //   const ObColumnSchemaV2 *orig_col_schema = NULL;
      //   if (OB_ISNULL(hidden_col_schema)) {
      //     ret = OB_ERR_UNEXPECTED;
      //     LOG_WARN("hidden col schema is null", KR(ret));
      //   } else if (OB_ISNULL(orig_col_schema = orig_table_schema.get_column_schema(
      //                                          hidden_col_schema->get_column_name()))) {
      //     ret = OB_ERR_UNEXPECTED;
      //     LOG_WARN("orig col schema is null", KR(ret));
      //   } else {
      //     ObSensitiveFieldItem orig_item(orig_col_schema->get_table_id(), orig_col_schema->get_column_id());
