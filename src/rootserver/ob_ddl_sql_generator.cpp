                                                   ...
                                                   ObSqlString &sql_string)
{
  ...
  if (OB_SUCC(ret)) {
    const char *priv_str = to_cstring(ObHexEscapeSqlStr(priv_string.string().ptr(), false, false));
    const char *sensitive_rule_str = to_cstring(ObHexEscapeSqlStr(need_priv.sensitive_rule_.ptr(), false, false));
    const char *user_name_str = to_cstring(ObHexEscapeSqlStr(account.user_name_.ptr(), false, false));
    if (0 == account.host_name_.compare(OB_DEFAULT_HOST_NAME)) {
      ...
      const char *host_name_str = to_cstring(ObHexEscapeSqlStr(account.host_name_.ptr(), false, false));
