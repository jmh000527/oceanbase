...
public:
  ...
  TO_STRING_KV(KP(this), K_(create_type), K_(type), KP_(algo_data),
              ...
              K_(mem_check_cnt), K_(is_mem_limited), K_(dump_info), K_(snapshot_key_prefix), K_(replace_scn));
private:
  void *get_incr_index();
  void *get_snap_index();
...
private:
   ...
   * we can't get scn from snapshot_key_prefix_ because it is invalid in some cases like BQ
   */
  SCN replace_scn_;
