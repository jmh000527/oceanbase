...
int ObInnerTableSchema::all_region_network_bandwidth_limit_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_SYS_TABLEGROUP_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_ALL_REGION_NETWORK_BANDWIDTH_LIMIT_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(2);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_ALL_REGION_NETWORK_BANDWIDTH_LIMIT_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    ADD_COLUMN_SCHEMA("src_region", //column_name
      ++column_id, //column_id
      1, //rowkey_id
      0, //index_id
      0, //part_key_pos
  ...
  if (OB_SUCC(ret)) {
    ADD_COLUMN_SCHEMA("dst_region", //column_name
      ++column_id, //column_id
      2, //rowkey_id
      0, //index_id
      0, //part_key_pos
  ...
  if (OB_SUCC(ret)) {
    ADD_COLUMN_SCHEMA("max_bw", //column_name
      ++column_id, //column_id
      0, //rowkey_id
      0, //index_id
      0, //part_key_pos
  ...
  table_schema.set_progressive_merge_round(1);
  table_schema.set_storage_format_version(3);
  table_schema.set_tablet_id(OB_ALL_REGION_NETWORK_BANDWIDTH_LIMIT_TID);
  table_schema.set_micro_index_clustered(false);
  table_schema.set_aux_lob_meta_tid(OB_ALL_REGION_NETWORK_BANDWIDTH_LIMIT_AUX_LOB_META_TID);
  table_schema.set_aux_lob_piece_tid(OB_ALL_REGION_NETWORK_BANDWIDTH_LIMIT_AUX_LOB_PIECE_TID);
