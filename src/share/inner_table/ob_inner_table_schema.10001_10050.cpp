...
int ObInnerTableSchema::all_virtual_processlist_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    ObObj top_trace_id_default;
    top_trace_id_default.set_varchar(ObString::make_string(""));
    ADD_COLUMN_SCHEMA_T("top_trace_id", //column_name
      ++column_id, //column_id
      0, //rowkey_id
      0, //index_id
      0, //part_key_pos
      ...
      true, //is_nullable
      false, //is_autoincrement
      top_trace_id_default,
      top_trace_id_default); //default_value
  }
  if (OB_SUCC(ret)) {
