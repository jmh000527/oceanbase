...
int ObInnerTableSchema::dba_ob_ccl_rules_ora_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_INVALID_ID);
  table_schema.set_database_id(OB_ORA_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_DBA_OB_CCL_RULES_ORA_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(0);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_DBA_OB_CCL_RULES_ORA_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(           SELECT           TENANT_ID,           CCL_RULE_ID,           CCL_RULE_NAME,           AFFECT_USER_NAME,           AFFECT_HOST,           AFFECT_FOR_ALL_DATABASES,           AFFECT_FOR_ALL_TABLES,           AFFECT_DATABASE,           AFFECT_TABLE,           AFFECT_DML,           AFFECT_SCOPE,           CCL_KEYWORDS,           MAX_CONCURRENCY         FROM SYS.ALL_VIRTUAL_CCL_RULE_REAL_AGENT )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::dba_mview_deps_ora_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       D.DATABASE_NAME AS MVIEW_OWNER,       B.TABLE_NAME    AS MVIEW_NAME,       COALESCE(EDB.DATABASE_NAME, RDB.DATABASE_NAME, TDB.DATABASE_NAME) AS DEP_OWNER,       COALESCE(C.TABLE_NAME, R.ROUTINE_NAME, T.TYPE_NAME) AS DEP_NAME,       CASE A.P_TYPE         WHEN 1 THEN 'TABLE'         WHEN 8 THEN 'VIEW'         WHEN 9 THEN 'FUNCTION'         WHEN 4 THEN 'TYPE'         ELSE 'INVALID TYPE'       END AS DEP_TYPE     FROM SYS.ALL_VIRTUAL_MVIEW_DEP_REAL_AGENT A     JOIN SYS.ALL_VIRTUAL_TABLE_REAL_AGENT B         ON A.MVIEW_ID = B.TABLE_ID     JOIN SYS.ALL_VIRTUAL_DATABASE_REAL_AGENT D         ON B.DATABASE_ID = D.DATABASE_ID      LEFT JOIN SYS.ALL_VIRTUAL_TABLE_REAL_AGENT C         ON A.P_OBJ = C.TABLE_ID     LEFT JOIN SYS.ALL_VIRTUAL_DATABASE_REAL_AGENT EDB         ON C.DATABASE_ID = EDB.DATABASE_ID      LEFT JOIN SYS.ALL_VIRTUAL_ROUTINE_REAL_AGENT R         ON A.P_OBJ = R.ROUTINE_ID     LEFT JOIN SYS.ALL_VIRTUAL_DATABASE_REAL_AGENT RDB         ON R.DATABASE_ID = RDB.DATABASE_ID      LEFT JOIN SYS.ALL_VIRTUAL_TYPE_REAL_AGENT T         ON A.P_OBJ = T.TYPE_ID     LEFT JOIN SYS.ALL_VIRTUAL_DATABASE_REAL_AGENT TDB         ON T.DATABASE_ID = TDB.DATABASE_ID     WHERE         (A.P_TYPE IN (1, 8) AND BITAND(TRUNC(C.TABLE_MODE / POWER(2,24)), 1) = 0)     OR (A.P_TYPE = 9)     OR (A.P_TYPE = 4); )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
