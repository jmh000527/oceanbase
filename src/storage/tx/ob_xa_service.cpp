                          ...
                          const uint64_t data_version)
{
  ...
  if (OB_UNLIKELY(xid.empty()) ||
    ...
    if (OB_FAIL(xa_start_(xid, flags, timeout_seconds, session_id, client_sid, tx_param, tx_desc, data_version))) {
                           ...
                           const uint64_t data_version)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    if (is_first_xa_start) {
      ...
      if (OB_FAIL(MTL(ObTransService *)->acquire_tx(tx_desc, session_id, client_sid, data_version))) {
