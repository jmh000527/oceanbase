    ...
    const common::ObIArray<int32_t> &cg_idxs)
{
  ...
  if (OB_FAIL(init_pre_check(schema_column_count, schema_rowkey_cnt, cols_desc,
    ...
    LOG_WARN("failed to init compat version", KR(ret));
      ...
      false /*is_cs_replica_compat*/, false /*is_delete_insert_table*/, false/*is_global_index_table*/, false/*mv_major_refresh_tablet*/))) { // init basic info
    ...
    const bool is_delete_insert_table)
{
  ...
  if (OB_FAIL(init_pre_check(schema_column_count, schema_rowkey_cnt, cols_desc,
    ...
    LOG_WARN("failed to init compat version", KR(ret));
      ...
      false /*is_cs_replica_compat*/, is_delete_insert_table, false/*is_global_index_table*/, false/*mv_major_refresh_tablet*/))) { // init basic info
    ...
    const bool is_mv_major_refresh_tablet)
{
  ...
  if (OB_SUCC(ret)) {
    init_basic_info(schema_column_count, schema_rowkey_cnt, is_oracle_mode,
                    ...
                    is_global_index_table, is_mv_major_refresh_tablet); // init basic info
