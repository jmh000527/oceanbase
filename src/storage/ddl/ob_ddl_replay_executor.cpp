...
int ObSplitStartReplayExecutor::do_replay_(ObTabletHandle &handle)
{
  ...
  LOG_INFO("finish replay tablet split start log", K(ret), K(can_skip), K(is_data_split_finished), K(scn_), KPC(log_));
  return ret;
}
...
int ObSplitFinishReplayExecutor::do_replay_(ObTabletHandle &handle)
{
  ...
  if (OB_TABLET_STATUS_NO_NEED_TO_SPLIT == ret) {
    ...
    FLOG_INFO("finish replay tablet split finish log", K(ret), K(can_skip), K(is_data_split_finished), K(scn_), KPC(log_),
