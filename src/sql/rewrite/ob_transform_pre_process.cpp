                                                          ...
                                                          bool is_trans_scope_temp_table)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(ctx_->expr_factory_->create_raw_expr(T_FUN_GET_TEMP_TABLE_SESSID, expr_temp_table_ssid))) {
      ...
      if (OB_FAIL(expr_temp_table_ssid->set_param_expr(temp_table_type))) {
        LOG_WARN("fail to set param expr", K(ret));
      }
    }
  }
                                                         ...
                                                         const ObColumnSchemaV2 &column_schema)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_SUCC(ret)) {
      if (OB_FAIL(session_label_expr->set_param_expr(policy_name_expr))) {
        LOG_WARN("fail to set param expr", K(ret));
      }
    }
  }
  ...
  if (OB_SUCC(ret)) {
    ...
    if (NULL != (col_item = stmt.get_column_item_by_id(table_item.table_id_, column_schema.get_column_id()))) {
      ...
      if (OB_FAIL(ObRawExprUtils::build_column_expr(*expr_factory, column_schema, session_info, col_expr))) {
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_SUCC(ret)) {
      if (OB_FAIL(label_cmp_expr->set_param_exprs(col_expr, session_label_expr))) {
                                                ...
                                                ObString &predicate_str)
{
  ...
  if (OB_ISNULL(ctx_)
    ...
    } else if (OB_FAIL(udf_expr->set_param_exprs(object_schema_expr, object_name_expr))) {
      LOG_WARN("failed to set param exprs", K(object_schema_expr), K(object_name_expr), K(ret));
                                                 ...
                                                 ObDMLStmt *stmt, bool &trans_happened)
{
  ...
  } else if (OB_FAIL(ObTransformUdtUtils::transform_udt_dml_stmt(ctx_, stmt, trans_happened))) {
    LOG_WARN("failed to do udt dml transform", K(ret));
  }
  return ret;
}
                                                       ...
                                                       bool& trans_happened)
{
  ...
  } else if (OB_FAIL(object_expr->init_param_exprs(select_items.count()))) {
    LOG_WARN("failed to init param exprs", K(ret));
  } else {
    pl::ObPLINS *ns = NULL;
