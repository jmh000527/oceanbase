    ...
    const ObTableSchema &orig_table_schema)
{
  ...
  if (OB_UNLIKELY(T_ALTER_SUBPARTITION_EXCHANGE != node.type_)) {
    ...
    LOG_WARN("get data version failed", K(ret), K(session_info_->get_effective_tenant_id()));
  } else if (OB_UNLIKELY(tenant_data_version < DATA_VERSION_4_3_5_3)) {
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("version lower than 4.3.5.3 does not support subpartition exchange", KR(ret), K(tenant_data_version));
    LOG_USER_ERROR(OB_NOT_SUPPORTED, "subpartition exchange of version lower than 4.3.5.3 is");
