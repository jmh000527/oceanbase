      ...
      if (privs_node != NULL && priv_level_node != NULL && users_node != NULL) {
        ...
        if (OB_SUCC(ret)) {
          ...
          if (OB_ISNULL(allocator_)) {
            ...
            ret = OB_NOT_SUPPORTED;
            LOG_WARN("grammar is not support when MIN_DATA_VERSION is below DATA_VERSION_4_3_5_3", K(ret));
  ...
  if (OB_SUCC(ret)) {
    if (priv_object_node->value_ == 1) {
      ...
      if (compat_version < DATA_VERSION_4_3_5_3) {
        ret = OB_NOT_SUPPORTED;
        LOG_WARN("grant on sensitive rule is not support below DATA_VERSION_4_3_5_3", K(ret));
      } else {
        grant_level = OB_PRIV_SENSITIVE_RULE_LEVEL;
      }
    } else {
