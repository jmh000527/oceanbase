          ...
          if (OB_SUCC(ret)) {
            if (is_oracle_mode()) {
              ...
              if (node->num_child_ != 1) {
                ...
                ret = OB_NOT_SUPPORTED;
                LOG_WARN("cluster version is less than 4.3.5.5, cast as array not supported", K(ret), K(cluster_version));
                LOG_USER_ERROR(OB_NOT_SUPPORTED, "cluster version is less than 4.3.5, cast as array");
...
int ObRawExprResolverImpl::process_sql_udt_attr_access_node(const ParseNode *node, ObRawExpr *&expr)
{
  ...
  if (OB_ISNULL(node)) {
    ...
    } else if (OB_FAIL(sys_attr_access->init_param_exprs(node->num_child_))) {
      LOG_WARN("failed to init param exprs", K(ret));
    } else {
...
int ObRawExprResolverImpl::process_sys_connect_by_path_node(const ParseNode *node, ObRawExpr *&expr)
{
  ...
  } else if (!(ctx_.session_info_->is_varparams_sql_prepare() || OB_NOT_NULL(ctx_.secondary_namespace_))
  ...
  } else if (OB_FAIL(path_expr->set_param_exprs(param1, param2))) {
    LOG_WARN("fail to set param exprs", KPC(param1), KPC(param2));
  } else {
...
int ObRawExprResolverImpl::process_connect_by_root_node(const ParseNode &node, ObRawExpr *&expr)
{
  ...
  if (node.num_child_ != 1
    ...
    ret = OB_ERR_CBY_PSEUDO_COLUMN_NOT_ALLOWED;
    LOG_WARN("Specified pseudocolumn or operator not allowed here", KPC(child_expr), K(ret));
  } else if (OB_FAIL(root_expr->set_param_expr(child_expr))) {
    LOG_WARN("fail to set param expr", KPC(root_expr), KPC(child_expr), K(ret));
  } else {
    expr = root_expr;
  }
  return ret;
...
int ObRawExprResolverImpl::process_prior_node(const ParseNode &node, ObRawExpr *&expr)
{
  ...
  } else if (OB_FAIL(prior_expr->set_param_expr(child_expr))) {
    LOG_WARN("fail to set param expr", KPC(prior_expr), KPC(child_expr), K(ret));
  } else {
    expr = prior_expr;
        ...
        if (OB_NOT_NULL(node->children_[0])) {
          ...
          for (int64_t i = 0; OB_SUCC(ret) && i < data_node->num_child_; i++) {  // 3 node in group
            ...
            if (OB_FAIL(ret) || OB_ISNULL(cur_node_kv)) {
              ...
              } else if (lib::is_oracle_mode() && ctx_.current_scope_ == T_PL_SCOPE) {
                ret = OB_ERR_PARSE_PLSQL;
                LOG_USER_ERROR(OB_ERR_PARSE_PLSQL, ",", ". ( * % & - + / at mod remainder rem <an exponent (**)> || multiset value");
              } else {
                ret = OB_ERR_UNEXPECTED;
  ...
  if (!is_win_expr_valid_scope(ctx_.current_scope_)
    ...
    if (T_FUN_COUNT == func_type
      ...
      if (OB_FAIL(ctx_.parents_expr_info_.add_member(IS_WINDOW_FUNC))) {
        ...
        } else if (OB_FAIL(floor_expr->set_param_expr(func_param))) {
          LOG_WARN("failed to set param expr", K(ret));
