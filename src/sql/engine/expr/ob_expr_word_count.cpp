...
namespace sql
{
ObExprWordCount::ObExprWordCount(ObIAllocator &allocator)
  : ObFuncExprOperator(allocator, T_FUN_SYS_WORD_COUNT, N_WORD_COUNT, MORE_THAN_ZERO, VALID_FOR_GENERATED_COL, NOT_ROW_DIMENSION)
{
  need_charset_convert_ = false;
}

int ObExprWordCount::calc_result_typeN(ObExprResType &type,
                                       ObExprResType *types,
                                       int64_t param_num,
...
int ObExprWordCount::calc_resultN(ObObj &result,
    ...
    ObExpr &rt_expr) const
{
  ...
  if (OB_UNLIKELY(rt_expr.arg_cnt_ < 1) || OB_ISNULL(rt_expr.args_)) {
    ...
    rt_expr.eval_func_ = generate_word_count;
  }
  return ret;
}

/*static*/ int ObExprWordCount::generate_word_count(
