...
ObExprCheckCatalogAccess::ObExprCheckCatalogAccess(ObIAllocator &alloc)
    :ObFuncExprOperator(alloc, T_FUN_SYS_CHECK_CATALOG_ACCESS, N_CHECK_CATALOG_ACCESS, 1,
                        NOT_VALID_FOR_GENERATED_COL, NOT_ROW_DIMENSION)
{
}

ObExprCheckCatalogAccess::~ObExprCheckCatalogAccess()
{
}

int ObExprCheckCatalogAccess::calc_result_type1(ObExprResType &type,
                                                ObExprResType &type1,
...
int ObExprCheckCatalogAccess::eval_check_catalog_access(const ObExpr &expr,
                                                        ObEvalCtx &ctx,
                                                        ObDatum &expr_datum)
{
  int ret = OB_SUCCESS;
  ObDatum *catalog = NULL;
...
int ObExprCheckCatalogAccess::cg_expr(ObExprCGCtx &op_cg_ctx, const ObRawExpr &raw_expr, ObExpr &rt_expr) const
{
  UNUSED(raw_expr);
  UNUSED(op_cg_ctx);
  rt_expr.eval_func_ = ObExprCheckCatalogAccess::eval_check_catalog_access;
  return OB_SUCCESS;
}
