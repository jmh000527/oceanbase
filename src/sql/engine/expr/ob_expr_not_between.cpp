...
int ObExprNotBetween::eval_not_between_vector(const ObExpr &expr,
                                              ObEvalCtx &ctx,
                           ...
                           ObExpr &rt_expr) const
{
  ...
  if (OB_UNLIKELY(3 != rt_expr.arg_cnt_) || OB_ISNULL(rt_expr.args_) ||
    ...
    if (OB_FAIL(ret)) {
      ...
      RowCmpFunc vec_cmp_func_1 = NULL;  // val < left
      RowCmpFunc vec_cmp_func_2 = NULL;  // right < val
      if (OB_ISNULL(vec_cmp_func_1 = VectorCmpExprFuncsHelper::get_row_cmp_func(
        ...
        LOG_WARN("The result of get_eval_vector_not_between_expr_cmp_func(left) is null.",
        ...
        LOG_WARN("The result of get_eval_vector_not_between_expr_cmp_func(right) is null.",
        ...
        rt_expr.eval_vector_func_ = eval_not_between_vector;
      }
...
#define DO_VECTOR_NOT_BETWEEN_ROW_CMP()                                    \
...
#define DO_VECTOR_NOT_BETWEEN_SET_RES()                                                \
  /*  Result priority: true > null > false  */                                         \
  if (OB_FAIL(ret)) {                                                                  \
  } else if (Stage == EvalBetweenStage::BETWEEN_LEFT) {                                \
    /* If the current calculation is left > val                                        \
    If the result is true, the subsequent calculation results are meaningless,         \
    ...
    only when the result is true will it be filled in. */                              \
    res_vec->set_int(i, 1);                                                            \
  }

template <typename LVec, typename RVec, typename ResVec,
          ObExprNotBetween::EvalBetweenStage Stage>
int ObExprNotBetween::inner_eval_not_between_vector(const ObExpr &expr,
                                                    ObEvalCtx &ctx,
                                                    ObBitVector &skip,
                                                    const EvalBound &bound)
{
