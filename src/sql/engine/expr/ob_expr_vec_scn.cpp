...
namespace sql
{
ObExprVecScn::ObExprVecScn(ObIAllocator &allocator)
  : ObFuncExprOperator(allocator, T_FUN_SYS_VEC_SCN, N_VEC_SCN, MORE_THAN_ZERO, VALID_FOR_GENERATED_COL, NOT_ROW_DIMENSION)
{
  need_charset_convert_ = false;
}

int ObExprVecScn::calc_result_typeN(ObExprResType &type,
                                       ObExprResType *types,
                                       int64_t param_num,
...
int ObExprVecScn::calc_resultN(ObObj &result,
    ...
    ObExpr &rt_expr) const
{
  ...
  if (OB_UNLIKELY(rt_expr.arg_cnt_ < 1) || OB_ISNULL(rt_expr.args_)) {
    ...
    rt_expr.eval_func_ = generate_vec_scn;
  }
  return ret;
}

/*static*/ int ObExprVecScn::generate_vec_scn(
