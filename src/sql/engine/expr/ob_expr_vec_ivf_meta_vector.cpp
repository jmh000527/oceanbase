...
namespace sql
{
ObExprVecIVFMetaVector::ObExprVecIVFMetaVector(ObIAllocator &allocator)
  : ObFuncExprOperator(allocator, T_FUN_SYS_VEC_IVF_META_VECTOR, N_VEC_IVF_META_VECTOR, MORE_THAN_ZERO, VALID_FOR_GENERATED_COL, NOT_ROW_DIMENSION)
{
  need_charset_convert_ = false;
}

int ObExprVecIVFMetaVector::calc_result_typeN(ObExprResType &type,
                                       ObExprResType *types,
                                       int64_t param_num,
...
int ObExprVecIVFMetaVector::calc_resultN(ObObj &result,
...
int ObExprVecIVFMetaVector::cg_expr(
    ...
    ObExpr &rt_expr) const
{
  ...
  if (OB_UNLIKELY(rt_expr.arg_cnt_ < 1) || OB_ISNULL(rt_expr.args_)) {
    ...
    rt_expr.eval_func_ = generate_meta_vector;
  }
  return ret;
}

int ObExprVecIVFMetaVector::generate_meta_vector(
    const ObExpr &expr,
    ObEvalCtx &eval_ctx,
    ObDatum &expr_datum)
{
