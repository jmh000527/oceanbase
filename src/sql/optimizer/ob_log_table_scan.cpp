                                              ...
                                              ObRawExpr *&real_expr)
{
  ...
  } else if (OB_FAIL(build_column_expr(opt_ctx->get_expr_factory(), *col_schema, mapping_col_expr))) {
...
int ObLogTableScan::filter_before_index_back_set()
{
  ...
  if (index_back_ && !is_vec_idx_scan_post_filter() && !is_vec_adaptive_scan()) {
  ...
  if (OB_NOT_NULL(table_opt_info_)) {
    ...
    if (OB_FAIL(ret)) {
      ...
      LOG_WARN("BUF_PRINTF fails", K(ret));
                            ...
                            static_cast<int64_t>(ceil(get_table_row_count()))))) {
      LOG_WARN("BUF_PRINTF fails", K(ret));
    } else if (OB_FAIL(BUF_PRINTF(NEW_LINE))) {
      LOG_WARN("BUF_PRINTF fails", K(ret));
    } else if (OB_FAIL(BUF_PRINTF(OUTPUT_PREFIX))) {
      LOG_WARN("BUF_PRINTF fails", K(ret));
                            ...
                            static_cast<int64_t>(ceil(get_phy_query_range_row_count()))))) {
      LOG_WARN("BUF_PRINTF fails", K(ret));
    } else if (OB_FAIL(BUF_PRINTF(NEW_LINE))) {
      LOG_WARN("BUF_PRINTF fails", K(ret));
    } else if (OB_FAIL(BUF_PRINTF(OUTPUT_PREFIX))) {
      LOG_WARN("BUF_PRINTF fails", K(ret));
                            ...
                            static_cast<int64_t>(ceil(get_logical_query_range_row_count()))))) {
      LOG_WARN("BUF_PRINTF fails", K(ret));
      ...
      LOG_WARN("BUF_PRINTF fails", K(ret));
                            ...
                            static_cast<int64_t>(ceil(get_index_back_row_count()))))) {
      LOG_WARN("BUF_PRINTF fails", K(ret));
    } else if (OB_FAIL(BUF_PRINTF(NEW_LINE))) {
      LOG_WARN("BUF_PRINTF fails", K(ret));
    } else if (OB_FAIL(BUF_PRINTF(OUTPUT_PREFIX))) {
...
int ObLogTableScan::allocate_granule_post(AllocGIContext &ctx)
{
  ...
  if (OB_FAIL(ret)) {
    // do nothing
  } else if (is_hnsw_vec_scan() || is_text_retrieval_scan() || is_vec_idx_scan_post_filter() || use_index_merge()
    ...
    // block GI is not supported for text retrieval scan, vec idx scan pre-filter and post filter and index merge
